...
# 创建使用新 Headless 模式的 API 服务器
RUN echo 'const express = require("express");const { renderMedia } = require("@remotion/renderer");console.log("Starting Remotion API Server...");const app = express();app.use(express.json());console.log("Express app created");app.get("/health", (req, res) => {console.log("Health check requested");res.json({status: "ok", message: "Remotion API Server is running", timestamp: new Date().toISOString(), memory: process.memoryUsage()});});app.post("/api/tasks", async (req, res) => {console.log("Render task requested:", req.body);try {const { fileName, outputLocation, composition, prone } = req.body;console.log("Task parameters:", { fileName, outputLocation, composition, prone });const outputPath = `/tmp/${fileName}`;console.log("Output path:", outputPath);const compositionConfig = {id: composition || "MyComposition", width: 640, height: 360, fps: 15, durationInFrames: 30};console.log("Starting render with composition:", compositionConfig);await renderMedia({composition: compositionConfig, serveUrl: "http://localhost:3000", codec: "h264", outputLocation: outputPath, inputProps: { prone: prone || 0 }, browserExecutable: "/usr/bin/chromium-browser", chromiumOptions: { args: ["--headless=new", "--no-sandbox", "--disable-setuid-sandbox", "--disable-dev-shm-usage", "--disable-accelerated-2d-canvas", "--no-first-run", "--no-zygote", "--single-process", "--disable-gpu"] }});console.log("Render completed successfully");res.json({success: true, message: "Video rendered successfully", taskId: Date.now(), fileName, outputPath, composition: compositionConfig});} catch (error) {console.error("Render error:", error);res.status(500).json({error: error.message, stack: error.stack});}});console.log("Routes defined");const PORT = process.env.PORT || 3000;app.listen(PORT, () => {console.log(`Remotion API Server running on port ${PORT}`);console.log(`Available endpoints:`);console.log(`- GET /health`);console.log(`- POST /api/tasks`);console.log(`Memory usage: ${JSON.stringify(process.memoryUsage())}`);});console.log("Server setup complete");' > server.js
